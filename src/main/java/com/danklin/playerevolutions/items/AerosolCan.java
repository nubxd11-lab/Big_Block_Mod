...
public class AerosolCan extends Item {

    public AerosolCan(Properties properties) {
        super(properties);
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
        ...
        if (!world.isRemote) {
            AerosolCanEntity canEntity = new AerosolCanEntity(world, player);
            canEntity.setItem(stack);
            canEntity.shoot(player, player.rotationPitch, player.rotationYaw, 0.0F, 1.2F, 1.0F);
            world.addEntity(canEntity);
        }
